...
public class SQLiteDatabase {
    ...
    public Cursor rawQuery(String sql, String[] makeArgListQueryString) throws SQLException {
        // TODO. improve logging. should use closure, it's really slow like this.
...
//                Log.v("SQLiteDatabase execSQL OK: " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName() + "\" " + sql);
    ...
    /**
     * Binds paramters to pre-compiled SQLite statement
     * @param statement SQLite statement
     * @param params parameters
     * @throws SQLException
     */
    private void bindParameters(SQLiteStatement statement, Object[] params) {
        for (int i = 0; i < params.length; i++) {
            Object param = params[i];

            int j = i + 1;
            if (param instanceof String) {
                statement.bindString(j, (String)param);
            } else if (param instanceof Integer || param instanceof Long || param instanceof Short || param instanceof Byte) {
                statement.bindLong(j, (Long)param);
            } else if (param == null) {
                statement.bindNull(j);
            } else if (param instanceof Double || param instanceof Float) {
                statement.bindDouble(j, (Double)param);
            } else if (param instanceof byte[]) {
                statement.bindBlob(j, (byte[]) param);
            } else if (param instanceof Boolean) {
                statement.bindLong(j, ((Boolean)param) ? 1L : 0L);
            } else {
                throw new IllegalArgumentException(String.format("Unsupported parameter type [%s]", param.getClass().getName()));
            }
        }
    }
     ...
     * @param statement Compiled statement
     * @param makeArgListQueryObject
     * @throws SQLException
     */
    public void execStatement(SQLiteStatement statement, Object[] makeArgListQueryObject) throws SQLException {
        // TODO. improve logging. should use closure, it's really slow like this.
//        Log.v("SQLiteDatabase execSQL: " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName() + "\" " + statement.toString());
        long timeNow = System.currentTimeMillis();
        long delta = 0;
        do {
            try {
                this.bindParameters(statement, makeArgListQueryObject);
                statement.execute();
//                Log.v("SQLiteDatabase execSQL OK: " + Thread.currentThread().getId() + " \"" + Thread.currentThread().getName() + "\" " + statement.toString());
                return;
            } catch (SQLiteException e) {
                if (isLockedException(e)) {
                    delta = System.currentTimeMillis() - timeNow;
